...
/**
 * Manages BLE connections to two MyCarv boot IMU sensors (left + right).
 *
 * Scans for devices advertising the MyCarv service UUID, connects to both,
...
class BleImuManager(private val context: Context) {
    ...
    private val gattCallback = object : BluetoothGattCallback() {
        ...
        override fun onServicesDiscovered(gatt: BluetoothGatt, status: Int) {
            ...
            if (service == null) {
                Log.e(TAG, "MyCarv service not found on ${gatt.device.name}")
                return
            }
        ...
        override fun onCharacteristicChanged(
            ...
            when (characteristic.uuid) {
                ...
                DUAL_CHAR_UUID -> {
                    ...
                    if (dual != null) {
                        when (gatt.device.name) {
                            "MyCarv-L" -> _leftDualImu.value = dual
                            "MyCarv-R" -> _rightDualImu.value = dual
                        }
                    }
                }
                TRI_CHAR_UUID -> {
                    ...
                    if (tri != null) {
                        when (gatt.device.name) {
                            "MyCarv-L" -> _leftTriSegment.value = tri
                            "MyCarv-R" -> _rightTriSegment.value = tri
                        }
                    }
                }
                THERM_CHAR_UUID -> {
