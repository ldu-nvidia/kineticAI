...
class MetricDetailViewModel(app: Application) : AndroidViewModel(app) {
    ...
    fun loadMetric(metricType: String) {
        viewModelScope.launch {
            ...
            _chart.value = when (metricType) {
                ...
                "avg_ski_iq" -> buildSkiIQOverTime(runs)
                "peak_ski_iq" -> buildSkiIQOverTime(runs, highlightMax = true)
    ...
    private fun buildSkiIQOverTime(
        runs: List<RunEntity>,
        highlightMax: Boolean = false,
    ): MetricChartData {
        if (runs.isEmpty()) return MetricChartData(
            title = "Ski:IQ Over Time", subtitle = "No runs yet", loaded = true,
        )
        ...
        return MetricChartData(
            title = if (highlightMax) "Peak Ski:IQ Progression" else "Avg Ski:IQ Over Time",
