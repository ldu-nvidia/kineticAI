 ...
 *   6. Running aggregate metrics + Ski:IQ computation
 */
class SkiAnalysisEngine(sampleRateHz: Float = 50f) {
    ...
    private fun updateMetrics(timestamp: Long, roll: Float, pitch: Float, lateralG: Float) {
        ...
        // ── Ski:IQ: weighted composite (0–200 scale) ──
        val skiIQ = computeSkiIQ(balanceScore, edgingScore, rotaryScore, pressureScoreVal)
     ...
     * Weights derived from Carv's published observation that G-force (pressure)
     * and edge angle (edging) are the strongest Ski:IQ predictors, with balance
     * and rotary as supporting factors.
     */
    private fun computeSkiIQ(
        balance: Float,
        edging: Float,
        rotary: Float,
        pressure: Float,
