# KineticAI — AI Ski Coach for Android

**Version 1.0.0**

A KineticAI-inspired ski coaching app that uses your Samsung Galaxy S26 Ultra's built-in sensors (accelerometer, gyroscope, magnetometer, barometer, GPS) to provide real-time technique feedback, post-run analysis, and a Kinetic Score score — all without any external hardware.

---

## What It Does

KineticAI turns your phone into a ski coach. Strap it to your body (chest pocket, thigh strap, or armband), hit Start, and ski. The app:
...
- Live Kinetic Score score (0–200)
- Foreground service keeps tracking with screen off

### Post-Run Analysis
- Kinetic Score score with level classification (Developing → Intermediate → Advanced → Expert)
...
- Compare Kinetic Score progression over time
- Season stats: total runs, avg/peak Kinetic Score, top speed, total distance, total vertical
...
## Kinetic Score Scoring (0–200)

Weighted composite of the 4 skill categories:

```
Kinetic Score = Balance × 0.20 + Edging × 0.35 + Rotary × 0.15 + Pressure × 0.30
```
...
Raw IMU @ 50 Hz (accel_xyz, gyro_xyz, pressure)
    ...
    │
    ├─ [7] Kinetic Score Computation (weighted composite)
    │
...
│   ├── SkiMetrics.kt             # Data classes for 12 metrics + Kinetic Score
...
└── ui/
    ...
    └── KineticAIAppRoot.kt          # Navigation graph
```
...
1. Open `kineticai/` in **Android Studio Ladybug** (2024.2+) or newer
2. Sync Gradle
...
1. **Dashboard** — See season stats (Kinetic Score, top speed, total vertical) and tap **Start Run**
...
5. **Post Analysis** — Review Kinetic Score, 4-skill breakdown, speed/edge/G-force charts, coaching insights
6. **History** — Browse all runs, track Kinetic Score improvement over time
...
- KineticAI's 12 metrics and how each is derived from IMU data
...
Personal project. Not affiliated with KineticAI or Motion Metrics Ltd.
